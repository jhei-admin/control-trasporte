...
class ApiSecurityAndIsolationTests(BaseFlotaTestCase):
    ...
    def test_api_gps_conductor_ignora_puntos_marcados_de_retorno_mientras_sigue_en_ida(self):
        hora_base = timezone.now() - timedelta(minutes=10)
        salida = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
        ...
        punto_muni_retorno = PuntoControl.objects.create(
            ruta=self.ruta_a,
            codigo="MUNI",
        ...
        MarcacionPunto.objects.create(
            registro_salida=salida,
            punto=punto_muni_retorno,
            hora_programada=hora_base + timedelta(minutes=48),
        )
        ...
        response = self.client.post(
            ...
            data=json.dumps(
                {
                    "lat": float(punto_muni_retorno.latitud),
                    "lng": float(punto_muni_retorno.longitud),
                    "precision": 10,
                }
            ),
            content_type="application/json",
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertNotEqual(data.get("motivo"), "punto_bloqueado")
        self.assertIsNone(
            MarcacionPunto.objects.get(
                registro_salida=salida,
                punto=punto_muni_retorno,
    ...
    def test_api_cola_contexto_mantiene_sobrepaso_si_vecino_ya_avanza_mas_en_misma_marcacion(self):
        hora_base = timezone.now() - timedelta(minutes=20)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
        ...
        punto_cole.orden = 2
        punto_cole.offset_minutos = 4
        punto_cole.confirma_avance = True
        punto_cole.fase = PuntoControl.FASE_IDA
        punto_cole.save(update_fields=["codigo", "nombre", "orden", "offset_minutos", "confirma_avance", "fase"])
        ...
        MarcacionPunto.objects.filter(registro_salida=salida_vecino, punto=punto_cole).update(
            hora_marcada=hora_base + timedelta(minutes=7),
        )
        MarcacionPunto.objects.filter(registro_salida=salida_actual, punto=punto_cole).update(
            hora_marcada=hora_base + timedelta(minutes=9),
        )
        ...
        UbicacionVehiculo.objects.create(
            vehiculo=self.vehiculo_2,
            latitud=Decimal(str(punto_apip.latitud)),
            longitud=Decimal(str(punto_apip.longitud)),
            velocidad=8,
            precision=10,
            ultimo_punto_evento_codigo=punto_apip.codigo,
            ultimo_punto_evento_orden=punto_apip.orden,
        ...
        self.assertEqual(data_actual["actual"]["punto_referencia_codigo"], punto_cole.codigo)
        self.assertEqual(data_actual["adelante"][0]["unidad"], self.vehiculo_2.codigo)
        self.assertEqual(data_actual["adelante"][0]["punto_referencia_codigo"], punto_apip.codigo)
        ...
        self.assertEqual(data_vecino["atras"][0]["punto_referencia_codigo"], punto_cole.codigo)

    def test_api_cola_contexto_devuelve_vecino_adelante_inmediato_primero_con_tres_unidades(self):
        hora_base = timezone.now().replace(hour=8, minute=0, second=0, microsecond=0)
        salida_lider = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
            hora_salida=hora_base,
        ...
        salida_intermedia = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_2,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
            hora_salida=hora_base + timedelta(minutes=3),
        ...
        salida_actual = RegistroSalida.objects.create(
            ...
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
            hora_salida=hora_base + timedelta(minutes=6),
            hora_real_salida=hora_base + timedelta(minutes=6),
            activo=True,
            en_cola=False,
        )
        ...
        data = response.json()
        self.assertTrue(data["ok"])
        self.assertEqual([item["unidad"] for item in data["adelante"]], [
            self.vehiculo_2.codigo,
            self.vehiculo_1.codigo,
        ])
        self.assertEqual(data["atras"], [])

    def test_api_cola_contexto_reordena_sobrepaso_en_puntos_de_paso_de_ida(self):
        hora_base = timezone.now().replace(hour=9, minute=20, second=0, microsecond=0)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
        ...
        punto_llam = self.punto_retorno
        punto_llam.codigo = "LLAM"
        punto_llam.nombre = "Llamagas"
        punto_llam.orden = 7
        punto_llam.requiere_marcacion = False
        punto_llam.confirma_avance = True
        punto_llam.fase = PuntoControl.FASE_IDA
        punto_llam.save(update_fields=["codigo", "nombre", "orden", "requiere_marcacion", "confirma_avance", "fase"])
        ...
        UbicacionVehiculo.objects.create(
            ...
            velocidad=10,
            precision=10,
            ultimo_punto_evento_codigo=punto_llam.codigo,
            ultimo_punto_evento_orden=punto_llam.orden,
            ultimo_punto_evento_at=timezone.now(),
        )
    ...
    def test_api_cola_contexto_no_reordena_por_evento_de_vecino_sin_senal(self):
        hora_base = timezone.now().replace(hour=9, minute=20, second=0, microsecond=0)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
        ...
        salida_siguiente = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_3,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
            hora_salida=hora_base + timedelta(minutes=6),
        ...
        for salida, minutos in ((salida_actual, 0), (salida_vecino, 3), (salida_siguiente, 6)):
            MarcacionPunto.objects.create(
                registro_salida=salida,
                punto=self.punto_salida,
                hora_marcada=hora_base + timedelta(minutes=minutos),
        ...
        UbicacionVehiculo.objects.create(
            vehiculo=self.vehiculo_2,
            latitud=Decimal(str(punto_muni.latitud)),
            longitud=Decimal(str(punto_muni.longitud)),
            velocidad=10,
            precision=10,
        ...
        data = response.json()
        self.assertTrue(data["ok"])
        self.assertEqual(data["actual"]["punto_referencia_codigo"], self.punto_salida.codigo)
        self.assertEqual(data["adelante"], [])
        self.assertEqual(data["atras"][0]["unidad"], self.vehiculo_3.codigo)
        self.assertEqual(data["atras"][0]["punto_referencia_codigo"], self.punto_salida.codigo)
        self.assertNotIn(self.vehiculo_2.codigo, [item["unidad"] for item in data["atras"]])

    def test_api_cola_contexto_primero_que_marca_mismo_punto_sigue_adelante_hasta_siguiente_referencia(self):
        hora_base = timezone.now() - timedelta(minutes=20)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
        ...
        punto_cole.nombre = "Colegio"
        punto_cole.offset_minutos = 4
        punto_cole.requiere_marcacion = True
        punto_cole.confirma_avance = True
        punto_cole.fase = PuntoControl.FASE_IDA
        punto_cole.save(update_fields=["codigo", "nombre", "offset_minutos", "requiere_marcacion", "confirma_avance", "fase"])
        ...
        MarcacionPunto.objects.filter(registro_salida=salida_vecino, punto=punto_cole).update(
            hora_marcada=hora_base + timedelta(minutes=6),
        )
        MarcacionPunto.objects.filter(registro_salida=salida_actual, punto=punto_cole).update(
            hora_marcada=hora_base + timedelta(minutes=8),
        )
        ...
        UbicacionVehiculo.objects.create(
            vehiculo=self.vehiculo_2,
            latitud=Decimal("-16.402100"),
            longitud=Decimal("-71.502100"),
            velocidad=8,
            precision=10,
        )
    ...
    def test_api_cola_contexto_primero_que_entra_misma_referencia_sigue_adelante_hasta_siguiente_punto(self):
        hora_base = timezone.now().replace(hour=11, minute=20, second=0, microsecond=0)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
        ...
        data_actual = response_actual.json()
        self.assertTrue(data_actual["ok"])
        self.assertEqual(data_actual["actual"]["punto_referencia_codigo"], punto_muni.codigo)
        self.assertEqual(data_actual["adelante"][0]["unidad"], self.vehiculo_2.codigo)
        self.assertEqual(data_actual["adelante"][0]["punto_referencia_codigo"], punto_muni.codigo)
    ...
    def test_api_cola_contexto_avanza_a_siguiente_punto_de_paso_en_misma_fase(self):
        hora_base = timezone.now().replace(hour=12, minute=0, second=0, microsecond=0)
        salida_actual = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
    ...
    def test_api_cola_contexto_no_promueve_contexto_interno_sobre_marcacion_real_posterior(self):
        hora_base = timezone.now().replace(hour=23, minute=5, second=0, microsecond=0)
        salida_adelante = RegistroSalida.objects.create(
            vehiculo=self.vehiculo_1,
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
        ...
        salida_actual = RegistroSalida.objects.create(
            ...
            ruta=self.ruta_a,
            fecha=timezone.localdate(),
            hora_salida=hora_base + timedelta(minutes=2),
            hora_real_salida=hora_base + timedelta(minutes=2),
            activo=True,
            en_cola=False,
        )
        ...
        for salida, minutos in ((salida_adelante, 0), (salida_actual, 2)):
            MarcacionPunto.objects.create(
                registro_salida=salida,
                punto=self.punto_salida,
                hora_marcada=hora_base + timedelta(minutes=minutos),
        ...
        UbicacionVehiculo.objects.create(
            ...
            velocidad=5,
            precision=10,
            ultimo_punto_evento_codigo="ZAMA",
            ultimo_punto_evento_orden=40,
            ultimo_punto_evento_at=timezone.now(),
        )
        UbicacionVehiculo.objects.create(
            vehiculo=self.vehiculo_2,
        ...
        self.assertEqual(data_actual["actual"]["punto_referencia_codigo"], punto_cole.codigo)
        self.assertEqual(data_actual["adelante"], [])
        self.assertEqual(data_actual["atras"][0]["unidad"], self.vehiculo_1.codigo)
        self.assertEqual(data_actual["atras"][0]["punto_actual_codigo"], self.punto_salida.codigo)
